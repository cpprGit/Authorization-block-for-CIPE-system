...
class SearchReportService(override val serviceName: String): BaseXlsxReportService() {

    override val reportName: String
        get() = ReportNames.SEARCH_REPORT.value
    ...
    override fun createXlsxReport(params: Map<String, Deque<String>>): XSSFWorkbook {
        ...
        val searchResult = when (params["searchType"]?.first()) {
            "students" -> searchStudentsService.search(params)
            "projects" -> searchProjectsService.search(params)
            "projectRequests" -> searchProjectRequestsService.search(params)
            "mentors" -> {
                val newParams = params as MutableMap
                val paramValue = LinkedList<String>()
                paramValue.add(UserRoles.MENTOR.value)
                newParams.putIfAbsent("role", paramValue)
                searchUsersService.search(newParams)
            }
            "managers" -> {
                val newParams = params as MutableMap
                val paramValue = LinkedList<String>()
                paramValue.add(UserRoles.MANAGER.value)
                newParams.putIfAbsent("role", paramValue)
                searchUsersService.search(newParams)
            }
            "representatives" -> {
                val newParams = params as MutableMap
                val paramValue = LinkedList<String>()
                paramValue.add(UserRoles.REPRESENTATIVE.value)
                newParams.putIfAbsent("role", paramValue)
                searchUsersService.search(newParams)
            }
            "organisations" -> searchOrganisationsService.search(params)
            "activities" -> searchActivitiesService.search(params)
            "questionnaires" -> searchQuestionnairesService.search(params)
            else -> throw BadRequestException("No such search type")
        }
